...
vel_radio = np.loadtxt(fname = "/Users/Lillie/Desktop/Research Summer 2023/HI_spectra/RFGC719_HIspec_reduced.txt", skiprows=4, dtype = float, usecols=0)
flux_density = np.loadtxt(fname = "/Users/Lillie/Desktop/Research Summer 2023/HI_spectra/RFGC719_HIspec_reduced.txt", skiprows=4, dtype = float, usecols=1)
...
plt.legend()
plt.title("HI Line Test")
plt.show()

thres_array =  np.full((len(vel_radio), 1), _thres) # creating array of same length as vel_radio but just filled with _thres val
...
print("width = ", width) # 335.0355367155116
'''
...
intersection = first_line.intersection(second_line)  # getting intersection of
...
print(nparray)
width = nparray[0][0] - nparray[-1][0] # width of band (found from first and last intersection)
print("width = ", width) # 276.81014634677194
'''
